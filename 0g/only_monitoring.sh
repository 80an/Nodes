...
# Функция проверки статуса валидатора и уведомление при попадании в тюрьму
check_validator() {
  while true; do
    ...
    if [ "$jailed_status" = "true" ]; then
      echo -e "${B_RED}Валидатор в тюрьме! Остановка скрипта для анализа причины.${NO_COLOR}"
      send_telegram_alert "🚨 Валидатор в тюрьме! Скрипт остановлен для анализа причины. Пожалуйста, проверьте статус и примите меры."
      exit 0
    fi
