...
while true; do
  ...
  case $choice in
    ...
    6)
      ...
      source <(wget -qO- 'https://raw.githubusercontent.com/80an/Nodes/refs/heads/main/0g/monitoring_validator.sh')
      echo
      ;;
    7)
      echo
      echo "⛔ Отключаем мониторинг валидатора..."
      pkill -f monitoring_validator.sh && echo "Мониторинг остановлен." || echo "Процесс не был запущен."
      echo
      ;;
    8)
